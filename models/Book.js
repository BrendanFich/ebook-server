...
class Book {
  ...
  createBookFromFile(file) {
    ...
    this.originalName = originalname // 原文件名
  }
          ...
          if (!title) {
            ...
            const handleGetImage = (err, file, mimeType) => {
              if (err) {
                ...
                const suffix = mimeType.split('/')[1] // 一般为 "image/jpeg"
  ...
  parseContents(epub) {
    ...
    function flatten(array) {
      return [].concat(
        ...array.map((item) => {
          if (item.navPoint && item.navPoint > 0) {
    ...
    const nxcFilePath = Book.genPath(`${this.unzipPath}/${getNcxFilePath()}`)
    if (fs.existsSync(nxcFilePath)) {
      return new Promise((resolve, reject) => {
        const xml = fs.readFileSync(nxcFilePath, 'utf-8')
        const dir = path.dirname(nxcFilePath).replace(UPLOAD_PATH, '')
    ...
    if (contents) {
      ...
      contents.forEach((i) => {
        ...
        if (!i.pid) {
          ...
          let parent = contents.find((chapter) => {
            return chapter.navId === i.pid
          })
